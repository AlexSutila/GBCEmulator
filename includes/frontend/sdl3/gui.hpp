...
constexpr SDL_WindowFlags kDetachedDialogWindowFlags = SDL_WINDOW_HIDDEN | SDL_WINDOW_BORDERLESS |
...
class GbcImGui {
  ...
  static constexpr std::string_view bios_filters = "BIOS files (*.bin){.bin},All files (*.*){.*}";
  ...
  static constexpr std::array<std::string_view, GK_COUNT> general_labels{
      "FF Toggle", "FF (Hold)", "Vol Up", "Vol Down", "Monochrome", "Quicksave", "Quickload"};
  ...
  static constexpr std::array<ThirdPartyProject, 11> kThirdPartyProjects{
      {
       ...
       {"mbedTLS", "https://www.trustedfirmware.org/projects/mbed-tls",
           "Apache 2.0 OR GPL 2.0 or later license"},
...
public:
  ...
  static void build_savestate_manager_window(
      ...
      const SavestateManagerCallbacks &callbacks, bool fill_viewport = false);

  static void push_notification(UiState &state, LogLevel level, const std::string &type,
...
private:
  ...
  std::array<ImGuiContextState, static_cast<std::size_t>(DialogId::Count)> detached_dialogs_{};

  void init_context(ImGuiContextState &ctx, SDL_Window *window, SDL_Renderer *renderer,
                    bool owns_window);
  ...
  [[nodiscard]] const ImGuiContextState *find_context_for_window(Uint32 window_id) const;
