...
void APU::register_mmio() {
  ...
  audio_registers[0x14].configure(power_on_nr50,
                                [this](const byte_t value) { nr50 = value; });
  audio_registers[0x15].configure(power_on_nr51,
                                [this](const byte_t value) { nr51 = value; });
...
void APU::clock_ch3_length() {
  if (const bool length_enabled = (nr34 & 0x40) != 0;
    !length_enabled || ch3_length_counter == 0) return;
  --ch3_length_counter;
...
void APU::clock_ch4_length() {
  if (const bool length_enabled = (nr44 & 0x40) != 0;
    !length_enabled || ch4_length_counter == 0) return;
  --ch4_length_counter;
...
float APU::channel3_sample() const {
  ...
  // center around 0
  const float s = (static_cast<float>(sample4) - 8.0f) / 8.0f; // ~[-1, +0.875]
  return s;
}
