...
static std::vector<byte_t> read_all_bytes(const fs::path &p) {
  ...
  if (!f)
    throw std::runtime_error("load_cart: failed to open ROM file: " +
                             p.string());

  const std::streamsize size = f.tellg();
  if (size < 0)
    throw std::runtime_error("load_cart: size read failed: " + p.string());

  std::vector<byte_t> buf(static_cast<std::size_t>(size));
  f.seekg(0, std::ios::beg);
  if (!f.read(reinterpret_cast<char *>(buf.data()), size)) {
    throw std::runtime_error("load_cart: failed to read ROM content: " +
                             p.string());
  }
  return buf;
}
