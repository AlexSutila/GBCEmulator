...
static std::string two_char_code(byte_t a, byte_t b) {
    auto printable = [](byte_t x) { return std::isprint(static_cast<unsigned char>(x)) != 0; };
...
std::string cgb_flag_desc(byte_t f) {
    switch (f) {
        case 0x80: return "CGB supported (DMG compatible)";
        case 0xC0: return "CGB only";
        default:   return "Not marked / older header";
    }
}

// SGB flag (0146)
std::string sgb_flag_desc(byte_t f) {
    if (f == 0x03) return "SGB functions supported";
...
std::string new_licensee_name(const std::string& two_chars) {
    ...
    for (auto& [k, v] : kNewLicensee) {
        if (two_chars == k) return std::string(v);
    }
    return "Unknown";
}
...
std::string rom_size_pretty(byte_t code) {
    ...
    switch (code) {
        case 0x52: return "1.1 MiB (72 ROM banks)";
        case 0x53: return "1.2 MiB (80 ROM banks)";
        case 0x54: return "1.5 MiB (96 ROM banks)";
        default:   return "Unknown";
    }
}
