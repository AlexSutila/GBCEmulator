...
// MBC2
// ---------------------------
// Internal 512 x 4-bit RAM at A000–A1FF with echoes; writes in 0000–3FFF use addr bit 8 to pick RAM-enable vs ROM-bank
...
public:
    ...
    byte_t read(addr_t const addr) override {
        ...
        if (addr >= 0xA000 && addr <= 0xBFFF) {
            if (!ram_enabled_) return open_bus();
    ...
    std::span<const byte_t> ram() const noexcept override { return std::span<const byte_t>(ram_.data(), ram_.size()); }
    std::span<byte_t>       ram() noexcept override { return std::span<byte_t>(ram_.data(), ram_.size()); }
