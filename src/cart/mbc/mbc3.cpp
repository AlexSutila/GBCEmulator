...
public:
    Mbc3(const std::span<const byte_t> rom, std::size_t const ram_bytes, bool const battery, bool const has_rtc)
        : rom_(rom), ram_(ram_bytes), battery_(battery), has_rtc_(has_rtc) {}

    // TODO: currently there is no mechanism to keep the clock ticking after the emulator is shut down.
    // It is reasonable to calculate the delta between now and "last power off" and apply the delta on boot. More
    // research needed
    ...
    byte_t read(addr_t const addr) override {
        if (addr <= 0x3FFF) return rom_at(0, addr);
        if (addr <= 0x7FFF) return rom_at(rom_bank_, addr - 0x4000);

        if (addr >= 0xA000 && addr <= 0xBFFF) {
            if (!ram_rtc_enabled_) return open_bus();
    ...
    void write(addr_t const addr, byte_t const val) override {
        ...
        if (addr <= 0x3FFF) {
            byte_t b = (val & 0x7F);
            if (b == 0) b = 1; // force 0b00 to 0b01, similar to MBC1
            rom_bank_ = b;
            return;
        }
        if (addr <= 0x5FFF) {
        ...
        if (addr >= 0xA000 && addr <= 0xBFFF) {
            ...
            if (sel_ >= 0x08 && sel_ <= 0x0C && has_rtc_) {
                ...
                // TODO: Pan Docs says set Halt before writing RTC regs, need to decide what to do
            }
        }
    }
...
private:
    ...
    struct RtcRegs {                    // Implements RTC Register 08-0C. Note: this is an abstraction, not a 1-to-1 replication of hw reg behavior
    ...
    static byte_t rtc_reg_read(const RtcRegs& r, byte_t const reg) {
        switch (reg) {
            case 0x08: return r.sec;  // 0-59
            case 0x09: return r.min;  // 0-59
            case 0x0A: return r.hour; // 0-23
            case 0x0B: return static_cast<byte_t>(r.day & 0xFF);    // DL (day low) bits
            // Bit 7 |  6   | 5 | 4 | 3 | 2 | 1 | 0
    ...
    static void rtc_reg_write(RtcRegs& r, byte_t const reg, byte_t const val) {
        switch (reg) {
            case 0x08: r.sec  = static_cast<byte_t>(val % 60); break;
            case 0x09: r.min  = static_cast<byte_t>(val % 60); break;
            case 0x0A: r.hour = static_cast<byte_t>(val % 24); break;
            case 0x0B: r.day  = static_cast<std::uint16_t>((r.day & 0x100) | val); break;
            case 0x0C: {
                r.day   = static_cast<std::uint16_t>((r.day & 0x0FF) | ((val & 0x01) << 8));   // Hope the math is right...
                r.halt  = (val & 0x40) != 0;
                r.carry = (val & 0x80) != 0;
                break;
            }
