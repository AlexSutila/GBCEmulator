...
public:
    Mbc5(std::span<const byte_t> const rom, std::size_t const ram_bytes, bool const battery, bool const rumble)
    ...
    byte_t read(addr_t const addr) override {
        ...
        if (addr >= 0xA000 && addr <= 0xBFFF) {
            ...
            const std::size_t bank = clamp_bank(ram_bank_, std::max<std::size_t>(1, ram_.size() / kRamBankSize));
    ...
    void write(addr_t const addr, byte_t const val) override {
        ...
        if (addr <= 0x2FFF) {
            rom_bank_ = (rom_bank_ & 0x100) | val; // low 8 bits        // Writing zero is OK unlike other MBCs
            return;
        }
        if (addr <= 0x3FFF) {
...
private:
    ...
    bool battery_{};
    bool rumble_{};                 // This controls whether rumble is enabled and is set by cartridge during init

    bool  ram_enabled_{false};      // 0b1010 enables like other MBCs. This is the result after the check
    std::uint16_t rom_bank_{0x001};     // 9-bit
    std::size_t   ram_bank_{0x00};
    bool rumble_on_{false};         // This is the physical state of rumble. Turning it on has no effect.
                                    // If we somehow port it to a handset then this can be hooked up to some motors
