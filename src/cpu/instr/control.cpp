...
void LR35902::init_control(lookup_table_t &lookup, bool *halted) {
  lookup.at(0x00) = make_unique<NOP>(&reg_file, bus);
  lookup.at(0x10) = make_unique<STOP>(&reg_file, bus);
  ...
  lookup.at(0x76) = make_unique<HALT>(&reg_file, bus, halted);
