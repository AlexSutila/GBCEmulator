...
function setupDynamicChromeVars() {
	...
	const update = () => {
		try {
			...
			const touchH = touchVisible
				? Math.ceil(touch.getBoundingClientRect().height)
				: 0;
...
function writeStatePayloadById(id, bytes) {
	const key = getStateEntryStorageKey(id);
	if (!key || !(bytes instanceof Uint8Array) || bytes.length === 0)
		return false;
	try {
...
globalThis.IroGBSaves = {
	...
	setPendingStateThumbnailRgba(bytes, width, height) {
		...
		saveRuntimeState.pendingStateThumb = rgbaThumbToDataUrl(
			bytes,
			w | 0,
			h | 0,
		);
	...
	saveActiveState(bytes) {
		...
		const now = Date.now();
		const req = saveRuntimeState.pendingSaveRequest || {
			kind: "quick",
			label: "",
		};
		saveRuntimeState.pendingSaveRequest = null;
		...
		const label =
			rawLabel || (kind === "manual" ? "Manual Save" : "Quick Save");
		const id = makeStateId();
		const pendingThumb =
			saveRuntimeState.pendingStateThumb || captureStateThumbnailDataUrl();
		saveRuntimeState.pendingStateThumb = "";
...
function saveStateHotkeys(map) {
	try {
		localStorage.setItem(STATE_HOTKEYS_KEY, JSON.stringify(map));
	} catch {}
}
...
async function ensureFflateLoaded() {
	...
	await new Promise((resolve, reject) => {
		...
		s.onload = resolve;
		s.onerror = () =>
			reject(new Error("Failed to load unzip library (fflate)."));
...
function pickRomFromMany(entries) {
	...
	if (!supportsDialog) {
		const list = entries
			.map((e, i) => `${i}: ${e.name} (${prettyBytes(e.size)})`)
			.join("\n");
	...
	onRuntimeInitialized() {
		...
		const requestQuicksave = (kind = "quick", label = "") => {
			...
			if (typeof Module._emscripten_request_quicksave === "function") {
				...
				setTemporaryStatus(
					kind === "manual" ? "Saving manual state…" : "Saving quick state…",
				);
		...
		function renderSavestateManager() {
			...
			for (const entry of states) {
				...
				item.type = "button";
				item.className =
					"savestate-item" + (entry.id === selectedStateId ? " selected" : "");
			...
			(e) => {
				...
				if (capturing?.kind === "pad") {
					...
					const other = ACTIONS.find(
						(x) => x.id !== action && keymap[x.id] === newCode,
					)?.id;
					...
					const action = capturing.id;
					const other = STATE_HOTKEY_ACTIONS.find(
						(x) => x.id !== action && stateHotkeys[x.id] === newCode,
					)?.id;
		...
		const loadRomBytes = async (bytes, nameForUi) => {
			...
			const fallbackTitle = basename(nameForUi || "ROM")
				.replace(/\.(gb|gbc)$/i, "")
				.trim();
			...
			if (auto) {
				(async () => {
					try {
						...
						console.warn(err);
						setStatusText(
							err && err.message ? err.message : "Failed to auto-load URL",
						);
						setTempDisabled(false);
					}
				})();
