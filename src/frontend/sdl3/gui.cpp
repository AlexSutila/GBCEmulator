...
void GbcImGui::init(const SDLHost& host) {
  ...
  if (!ImGui_ImplSDL3_InitForSDLRenderer(host.get_window(), host.get_renderer()))
...
void GbcImGui::render(UiState& state, SDLHost& host) {
  ...
  if (state.show_settings) build_settings_window(state, host);
  if (state.show_keybinds) build_keybinds_window(state);
}

// Returns true if the event was handled by the GUI and should be ignored by the game
    ...
    if (ImGui::BeginMenu("File")) {
      if (ImGui::MenuItem("Load ROM..."))
        ImGuiFileDialog::Instance()->OpenDialog(
            "RomFileDialog", "Choose a ROM file", rom_filters.data(), rom_sel_conf);
      ...
      if (ImGui::MenuItem("Select BIOS"))
        ImGuiFileDialog::Instance()->OpenDialog(
            "BiosFileDialog", "Choose a BIN file", bios_filters.data(), bios_sel_conf);
...
void GbcImGui::build_settings_window(UiState& state, SDLHost& host) {
  ...
  if (ImGui::Button("Refresh")) {
    ...
    state.current_audio_dev_idx =
        std::min(state.current_audio_dev_idx, static_cast<int>(state.audio_device_names.size()) - 1);
  }
  ImGui::End();
}
...
void GbcImGui::apply_keybind_preset(std::array<SDL_Keycode, 8>& array, const int keybind_preset_index) {
