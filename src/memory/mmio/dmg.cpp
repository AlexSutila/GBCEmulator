...
bool LCDCtrl::bg_win_en_priority() const { return (state & 0x1) != 0; }
...
bool LCDCtrl::obj_enable() const { return (state & 0x02) != 0; }
...
bool STAT::get_ly_eq_lyc() const { return (state & 0x04) != 0; }

StatModes STAT::get_mode() const {
  constexpr byte_t mode_mask = 0x03;
  const auto mode = static_cast<StatModes>(state & mode_mask);
  return mode;
}
